...
\chead{MTH 3345: Intermediate Logic}
\rhead{\today}
...
%\begin{problemgroup}
    \begin{problem}
        ...
        \end{enumerate}
    \end{problem}
    The statements in \textsc{FOL} can be translated to plain English as:
    \begin{enumerate}
        \item Max owned Scruffy at 2:00.
        \item Max fed Scruffy at 2:30.
        \item Max gave Scruffy to Claire at 3:00.
        \item 2:00 is earlier than 2:00.
    \end{enumerate}
%\end{problemgroup}
\vspace{.5em}
...
%\begin{problemgroup}
    ...
    \begin{center}
        \begin{tabular}{c|l|c|c}
            & \textsc{FOL} & Leibniz's & Wittgenstein's \\
            \hline
            1. & \texttt{Tet(lm(e))} & \textsc{True} & \textsc{False} \\
            2. & \texttt{fm(c)$=$c} & \textsc{True} & \textsc{True} \\
            3. & \texttt{bm(b)$=$bm(e)} & \textsc{True} & \textsc{True} \\
            4. & \texttt{FrontOf(fm(e), e)} & \textsc{False} & \textsc{True} \\
            5. & \texttt{LeftOf(fm(b), b)} & \textsc{False} & \textsc{False} \\
            6. & \texttt{SameRow(rm(c), c)} & \textsc{True} & \textsc{True} \\
            7. & \texttt{bm(lm(c))$=$lm(bm(c))} & \textsc{True} & \textsc{True} \\
            8. & \texttt{SameShape(lm(b), bm(rm(e)))} & \textsc{True} & \textsc{True} \\
            9. & \texttt{d$=$lm(fm(rm(bm(d))))} & \textsc{True} & \textsc{False} \\
            10. & \texttt{Between(b, lm(b), rm(b))} & \textsc{True} & \textsc{True} \\
        \end{tabular}
    \end{center}
